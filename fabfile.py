...
def symlink_nginx():
    ...
    with cd(env.path):
        run('ln -sf $PWD/server/nginx/{host}.conf '
            '{nginx_conf_dir}/fabrydb-{host}.conf'.format(**env))
...
def reload_supervisor():
    ...
    with cd(env.path):
        run('ln -sf $PWD/server/supervisor/{host}.ini '
            '{supervisor_conf_dir}/fabrydb-{host}.ini'.format(**env))
...
def reload_wsgi():
    ...
    pid = run('supervisorctl pid fabrydb-{0}'.format(env.host))
    run('kill -HUP {0}'.format(pid))
...
def upload_settings():
    ...
    if os.path.exists(local_path):
        remote_path = os.path.join(env.path, 'fabrydb/conf/local_settings.py')
